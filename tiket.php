...
$sql = "SELECT order_id, destinasi, email, tanggal_kunjungan, jumlah_tiket, transaction_status, total_bayar
        FROM pemesanan 
        ORDER BY id DESC"; // Mengambil entri terbaru berdasarkan ID tertinggi
  ...
  <div class="card shadow-lg">
    ...
    <div class="card-body">
      ...
      <table class="detail-table">
        <tr><th>Order ID</th><td> : <?php echo $data['order_id']; ?></td></tr>
        <tr><th>Destinasi</th><td> : <?php echo $data['destinasi']; ?></td></tr>
        <tr><th>Email</th><td> : <?php echo $data['email']; ?></td></tr>
        <tr><th>Tanggal Kunjungan</th><td> : <?php echo $data['tanggal_kunjungan']; ?></td></tr>
        <tr><th>Jumlah Tiket</th><td> : <?php echo $data['jumlah_tiket']; ?> Orang</td></tr>
        <tr><th>Total Bayar</th><td> : <?php echo $data['total_bayar']; ?></td></tr>
        <tr><th>Status Tiket</th><td>: 
          <?php
          if ($data['transaction_status'] == 3) echo "Pembayaran Sukses";
          elseif ($data['transaction_status'] == 2) echo "Pembayaran Pending";
          elseif ($data['transaction_status'] == 0) echo "Dibatalkan";
    ...
    <div class="card-footer text-center">
      <?php if ($data['transaction_status'] == 1): ?>
        <a href='./midtrans/examples/snap/checkout-process-simple-version.php?order_id=<?php echo $data['order_id']; ?>' class="btn btn-danger">Bayar Sekarang</a>
        <a href="cancel-tiket.php?order_id=<?php echo $data['order_id']; ?>" class="btn btn-danger" onclick="return confirm('Apakah Anda yakin ingin membatalkan tiket ini?')">Batalkan Tiket</a>
      <?php elseif ($data['transaction_status'] != 0): ?>
